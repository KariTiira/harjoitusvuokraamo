...
if (!empty($_POST)) {
    ...
    if($valid) {
        ...
        $sql = "UPDATE asiakas
                SET henkilotunnus = :henkilotunnus, etunimi = :etunimi, sukunimi = :sukunimi, lahiosoite = :lahiosoite, postinumero = :postinumero, postitoimipaikka = :postitoimipaikka, sahkoposti = :sahkoposti, puhelinnumero = :puhelinnumero
                WHERE asiakasID = :asiakasID";

        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':henkilotunnus', $henkilotunnus, PDO::PARAM_STR);
        $stmt->bindParam(':asiakasID', $asiakasID, PDO::PARAM_STR);
        ...
        $stmt->bindParam(':puhelinnumero', $puhelin, PDO::PARAM_STR);
        $stmt->execute();
                ...
                <div class="card card-body bg-light mt-3">
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="mt-3">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" value="<?php echo (!empty($sahkposti)) ? $sahkoposti : ''; ?>" name="sahkoposti" class="form-control <?php echo (!empty($sahkopostiError)) ? 'is-valid' : ''; ?>" id="inputSahkoposti">
